...
def main(args):
    ...
    if policy_config.name == "model_predictive_rl":
        ...
        trainer = VNRLTrainer(
            model, memory, device, policy, batch_size, optimizer, writer
        )
    explorer = Explorer(
        env, robot, device, writer, memory, policy.gamma, target_policy=policy
    )
    ...
    while episode < train_episodes:
        ...
        if episode != 0 and episode % checkpoint_interval == 0:
            ...
            save_every_checkpoint_rl_weight_file = (
                rl_weight_file.split(".")[0] + "_" + str(current_checkpoint) + ".pth"
            )
    ...
    if best_val_model is not None:
        ...
        logging.info(
            "Save the best val model with the reward: {}".format(best_val_reward)
        )
    explorer.run_k_episodes(
        env.case_size["test"], "test", episode=episode, print_failure=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="configs/icra_benchmark/mp_separate.py"
    )
