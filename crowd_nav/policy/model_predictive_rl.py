...
class ModelPredictiveRL(Policy):
        ...
        if self.linear_state_predictor:
            ...
            if self.share_graph_model:
                ...
                self.state_predictor = StatePredictor(
                    config, graph_model, self.time_step
                )
                self.model = [
                    graph_model,
                    self.value_estimator.value_network,
                ...
                self.state_predictor = StatePredictor(
                    config, graph_model2, self.time_step
                )
                self.model = [
                    graph_model1,
                    graph_model2,
    ...
    def load_state_dict(self, state_dict):
        if self.state_predictor.trainable:
            if self.share_graph_model:
                self.value_estimator.graph_model.load_state_dict(
                    state_dict["graph_model"]
                )
            else:
                self.value_estimator.graph_model.load_state_dict(
                    state_dict["graph_model1"]
                )
                self.state_predictor.graph_model.load_state_dict(
                    state_dict["graph_model2"]
                )

            self.value_estimator.value_network.load_state_dict(
                state_dict["value_network"]
            )
            self.state_predictor.human_motion_predictor.load_state_dict(
                state_dict["motion_predictor"]
            )
        else:
    ...
    def build_action_space(self, v_pref):
        ...
        for j, speed in enumerate(speeds):
            ...
            for i, rotation in enumerate(rotations):
                ...
                action_index = (
                    speed_index * self.sparse_rotation_samples + rotation_index
                )
    ...
    def estimate_reward(self, state, action):
        ...
        reaching_goal = (
            norm(end_position - np.array([robot_state.gx, robot_state.gy]))
            < robot_state.radius
        )
    ...
    def transform(self, state):
        ...
        """
        robot_state_tensor = torch.Tensor([state.robot_state.to_tuple()]).to(
            self.device
        )
        human_states_tensor = torch.Tensor(
            [human_state.to_tuple() for human_state in state.human_states]
        ).to(self.device)
